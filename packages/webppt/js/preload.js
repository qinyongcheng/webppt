 ...
 */
var preload = function(imgList, callback, timeout) {
	timeout = timeout || 2000;
	var $imgList = $.isArray(imgList) && imgList || $.isFunction(imgList) && imgList.call(this) || [];
	...
	for (var i = 0; i < total; i++) {
		...
		if ($img.is('img')) {
			...
			if ((!$img.css("background-image") || $img.css("background-image") == "none") && $img.attr(
					'data-src')) {
				imgages[num] = new Image();
				imgages[num].onload = imgages[num].onerror = (function($img) {
